...
architecture control_beh of control_unit is
  signal s_force_stall : std_logic := '0';              -- Internal signal to force the PC update to be stalled
begin
  ...
  --------------------------------------------------------------------------------
  pc_stall_proc: process(s_force_stall)
  begin
    if s_force_stall = '1' then
      o_PC_stall <= '1';
    else
      o_PC_stall <= '0';
    end if;
  ...
  -- i_ALUzero for branch instructions.
  --------------------------------------------------------------------------------
  opcode_decode : process (i_opcode, i_funct3, i_funct7, i_ALUzero)
  begin
    ...
    if i_opcode = "0110011" then
      ...
      o_data_write_en <= '0';                             --  No data is written to memory
    ...
    --------------------------------------------------------------------------------
      ...
      if i_funct3 = "010" then                         -- LOAD WORD INSTRUCTION
        s_force_stall <= '0';
      else                                                -- INVALID INSTRUCTION
        s_force_stall <= '1';
      end if;
